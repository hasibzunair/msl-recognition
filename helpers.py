...
class AttributesMeter(object):
    ...
    def get_f1_and_acc(self, mean_indexes=None):
        ...
        if self.f1_score_macros is None:
            self.f1_score_macros = np.array(
                [
                    f1_score(y_pred=self.preds[i], y_true=self.gts[i], average="macro")
                    for i in [0, 1] + list(range(self.attr_num))
                ]
            )
        ...
        return (
            ...
            self.acces_avg,
            np.mean(self.acces_avg[mean_indexes]),
            np.mean(self.f1_score_macros[mean_indexes]),
        )
