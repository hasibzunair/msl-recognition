...
class LeViT_UNet_128s(torch.nn.Module):
    ...
    def __init__(
        self,
        img_size=224,  ## aping: should change
        patch_size=16,
        in_chans=3,
        num_classes=9,
        embed_dim=[192],
        ...
        self.decoderBlock_1 = DecoderBlock(
            n + (128 + 256 + 384), 512
        )  # ->att+cnn:128+768=896  '128_256_384'
        ...
        self.segmentation_head = SegmentationHead(
            n // 8 + 128, self.num_classes, kernel_size=3, upsampling=2
        )
    ...
    def forward(self, x):
        ...
        x = self.block_1(
            x
        )  # torch.Size([1, 196, 128])--> torch.Size([1, 196, 128]) : CNN->Trans
        ...
        x = self.block_2(
            x
        )  # downsample + att torch.Size([1, 196, 128])-> torch.Size([1, 49, 256])
        x_num, x_len = x.shape[0], x.shape[1]
        # reshape: torch.Size([4, 14, 14, 256])
...
if __name__ == "__main__":
    for name in specification:
        ...
        print(
            "the number of parameters: %d ==> %.2f M"
