...
class BasicBlock(Module):
    ...
    def __init__(
        ...
        if stride == 1:
            ...
            if anti_alias_layer is None:
                self.conv1 = conv2d_ABN(
                    inplanes, planes, stride=2, activation_param=1e-3
                )
            else:
        ...
        self.se = (
            SEModule(planes * self.expansion, reduce_layer_planes) if use_se else None
        )
...
class TResNet(Module):
    def __init__(
        ...
        self.global_pool = nn.Sequential(
            OrderedDict([("global_pool_layer", global_pool_layer)])
        )
        self.num_features = (self.planes * 8) * Bottleneck.expansion
        if do_bottleneck_head:
            fc = bottleneck_head(
                self.num_features, num_classes, bottleneck_features=bottleneck_features
            )
        else:
